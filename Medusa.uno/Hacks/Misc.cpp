...
void Misc::watermark() noexcept
{
    ...
    std::ostringstream text; text << c_xor("joeware") << (config->misc.wm.showUsername ? " | " + namey : "") << (config->misc.wm.showTime ? " | " + fuck : "") << (config->misc.wm.showFps ? " | " + fps : "") << (config->misc.wm.showPing ? " | " + ping : "") << (interfaces->engine->isConnected() && config->misc.wm.showTicks ? " | " + std::to_string(tick) + " Ticks" : "");
...
void Misc::killMessage(GameEvent& event) noexcept
{
    ...
    {
    case 0:
        killMessage = skCrypt("joeware on top nigger");
        break;
    case 1:
        killMessage = skCrypt("KILLED BY TOP 1 JOEWARE WITH JS Rezolver.py");
        break;
    case 2:
        killMessage = skCrypt("ｗｅａｋ ｒａｔｓ ｓｅｎｔ ｔｏ ｈｅｌｌ ｂｙ ｔｅｅｎ ｗｏｌｆｓ ｂｂｃ");
        break;
    case 3:
        killMessage = skCrypt("A WEAK ESOTERIK MADE A MISTAKE WHEN HE BANNED MY FRIENDS SO I CRACKED HIS SKULL AND HIS PASTE");
        break;
    case 4:
        killMessage = skCrypt("A MOSQUITO LANDED ON MY SCREEN (you) SO I HAD TO SLAP HIM TO DEATH");
        break;
    case 5:
        killMessage = skCrypt("ALL DOGS HAD FUN BUT STOPPED LAUGHING WHEN THEY DIED");
        break;
    case 6:
        killMessage = skCrypt("𝒊 𝒌𝒊𝒍𝒍𝒆𝒅 𝒈𝒂𝒎𝒆𝒔𝒆𝒏𝒔𝒆 𝒏𝒐𝒘 𝒊 𝒌𝒊𝒍𝒍 𝒚𝒐𝒖");
        break;
    case 7:
        killMessage = skCrypt("𝗂 𝗁𝗏𝗁 𝖿𝗈𝗋 𝗆𝗒 𝗅𝗈𝗏𝖾𝖽 𝗈𝗇𝖾𝗌 <𝟥 𝗍𝗁𝗂𝗌 𝗁𝗌 𝟦 𝗎");
        break;
    case 8:
