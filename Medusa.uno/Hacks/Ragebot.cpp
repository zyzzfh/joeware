...
void runRagebot(UserCmd* cmd, Entity* entity, matrix3x4* matrix, Ragebot::Enemies target, std::array<bool, Hitboxes::Max> hitbox, Entity* activeWeapon, int weaponIndex, Vector localPlayerEyePosition, Vector aimPunch, int multiPointHead, int multiPointBody, int minDamage, float& damageDiff, Vector& bestAngle, Vector& bestTarget) noexcept
{
    ...
    for (size_t i = 0; i < hitbox.size(); i++)
    {
        ...
        for (auto& bonePosition : AimbotFunction::multiPoint(entity, matrix, hitbox, localPlayerEyePosition, i, multiPointHead, multiPointBody))
        {
            ...
            if (cfg.autoScope && activeWeapon->isSniperRifle() && !localPlayer->isScoped() && !activeWeapon->zoomLevel() && localPlayer->flags() & 1 && !(cmd->buttons & UserCmd::IN_JUMP))
         ...
         if (Tickbase::isPeeking = true)
         {
             if (cfg1[weaponIndex].autoStop && localPlayer->flags() & 1  && isSpreadEnabled->getInt() == 0) //& FL_ONGROUND &&!(cmd->buttons & UserCmd::IN_JUMP)
             {
                 if ((cfg1[weaponIndex].autoStopMod & 1 << 0) == 1 << 0)
                 {
                     ...
                     if (!cfg.accuracyBoost == 0.f)
                         const auto speedcoeffi = 1.f - cfg.accuracyBoost;
                 ...
                 if (cfg.autoScope && activeWeapon->isSniperRifle() && !localPlayer->isScoped() && !activeWeapon->zoomLevel() && localPlayer->flags() & 1)
                     cmd->buttons |= UserCmd::IN_ZOOM;
...
static bool CompareRecords(const optimized_adjust_data& first, const optimized_adjust_data& second) {
    ...
    auto first_pitch = Helpers::NormalizePitch(first.angles.x);
    auto second_pitch = Helpers::NormalizePitch(second.angles.x);
    ...
    if (fabs(first_pitch - second_pitch) > 15.0f)
        ...
        return first.origin.DistTo(LocalPlayer()->getAbsOrigin()) < second.origin.DistTo(LocalPlayer()->getAbsOrigin());
...
void Ragebot::run(UserCmd* cmd) noexcept
{
    ...
    if (!(config->ragebot.enabled && (cmd->buttons & UserCmd::IN_ATTACK || cfg.autoShot)))
        return;
    ...
    for (int i = 1; i <= interfaces->engine->getMaxClients(); ++i) {
        ...
        if (!entity || entity == localPlayer.get() || entity->isDormant() || !entity->isAlive() || !entity->isOtherEnemy(localPlayer.get()) && !cfg.friendlyFire || entity->gunGameImmunity())
            continue;
    ...
    for (const auto& target : enemies)
    {
        ...
        {
            auto update = player_records[i].empty() || entity->simulationTime() != entity->oldSimulationTime();
