...
struct Ray {
    void Init(Vector const& start1, Vector const& end, Vector const& mins, Vector const& maxs)
    {
        delta = end - start;

        worldAxisTransform = NULL;
        isSwept = (delta.length2D() != 0); //-V550

        extents = maxs - mins;
        extents *= 0.5f;
        isRay = (extents.length2D() < 1e-6);

        // Offset m_Start to be in the center of the box...
        startOffset = maxs + mins;
        startOffset *= 0.5f;
        start = start1 + startOffset;
        startOffset *= -1.0f;
    }
