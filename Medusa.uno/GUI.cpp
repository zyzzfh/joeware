...
GUI::GUI() noexcept
{
    ...
    if (PWSTR pathToFonts; SUCCEEDED(SHGetKnownFolderPath(FOLDERID_Fonts, 0, nullptr, &pathToFonts))) {
        ...
        fonts.normal15px = io.Fonts->AddFontFromFileTTF((path / "verdana.ttf").string().c_str(), 15.0f, &cfg, Helpers::getFontGlyphRanges());
        if (!fonts.normal15px)
            io.Fonts->AddFontDefault(&cfg);

        fonts.tahoma34 = io.Fonts->AddFontFromFileTTF((path / "verdana.ttf").string().c_str(), 34.0f, &cfg, Helpers::getFontGlyphRanges());
        if (!fonts.tahoma34)
            io.Fonts->AddFontDefault(&cfg);

        fonts.tahoma24 = io.Fonts->AddFontFromFileTTF((path / "verdana.ttf").string().c_str(), 22.0f, &cfg, Helpers::getFontGlyphRanges());
        if (!fonts.tahoma24)
            io.Fonts->AddFontDefault(&cfg);
...
void renderESPpreview(ImDrawList* draw, ImVec2 pos) noexcept
{
    ...
    auto sizeMed = ImGui::CalcTextSize(skCrypt("joeware"));
    ImGui::PopFont();
    ImGui::PushFont(gui->weaponIcons());
    ...
    if (currentCategory == 0)
    {
        ...
        {
            draw->AddText(gui->espFont(), 12.f, ImVec2{pos.x + 185 / 2 - (sizeMed.x / 2), pos.y + 25}, Helpers::calculateColor(config->esp.enemy.name), c_xor("joeware"));
        }
    ...
    if (currentCategory == 1)
    {
        ...
        {
            draw->AddText(gui->espFont(), 12.f, ImVec2{ pos.x + 185 / 2 - (sizeMed.x / 2), pos.y + 25 }, Helpers::calculateColor(config->esp.allies.name), c_xor("joeware"));
        }
...
void GUI::renderVisualsWindow() noexcept
{
   ...
   // }
    ...
    {
        ImGui::SameLine();
        ImGui::PushID("?1'thirdperson'1?tf is u looking in hex for 'fart");
        if (ImGui::Button("..."))
            ImGui::OpenPopup("ttt");
...
void GUI::renderMovementWindow() noexcept
{
    ...
    ImGui::SameLine();
    ImGui::PushID("stop using hex u dirty hoe,");
    if (ImGui::Button("..."))
        ImGui::OpenPopup("");
    if (ImGui::BeginPopup(""))
    {
